...
class AllJobs extends Component {
  ...
  changeEmployeeList = event => {
    this.setState(prevState => {
      ...
      const isAlreadySelected = prevState.employeeType.includes(event.target.id)

      // If it's already selected, remove it from the list
      if (isAlreadySelected) {
        return {
          employeeType: prevState.employeeType.filter(item => item !== event.target.id),
        }
      }
      return {
        employeeType: [...prevState.employeeType, event.target.id],
      }
    }, this.onGetJobDetails) // Call getJobs after updating the state
  }
      ...
      {employmentTypesList.map(eachItem => (
        <li className="li-container" key={eachItem.employmentTypeId}>
          <input
            ...
            type="checkbox"
            onChange={this.changeEmployeeList}
          />
      ...
      <>
        ...
        <div className="all-jobs-container">
          ...
          <div className="jobs-container">
            ...
            <p className="jol">{checkboxInputs}</p>
            <p className="jol"> {radioInput}</p>
          </div>
        </div>
      </>
    )
